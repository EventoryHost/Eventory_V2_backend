...
async function main() {
  ...
  // ============================================================
  section("Vendor step verification");
  ...
  await call({
    ...
    expect: (s, b) =>
      ...
      b.data.some((v) => v.id === stepVendorId && v.verificationStatus === "Rejected" && typeof v.completionPercent === "number"),
  });
  ...
  await db.collection("vendors").deleteMany({ id: { $in: [stepVendorId, skipVendorId] } });
