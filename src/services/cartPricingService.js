...
export async function computeCartQuote(cart) {
  ...
  const lineQuotes = items
    .map((item) => {
      ...
      const packagePriceTotal = round2(currentPrice * item.quantity);
      ...
      const milestones = computeLineMilestones(lineSubtotal, pkg, item.eventDetails?.date);
